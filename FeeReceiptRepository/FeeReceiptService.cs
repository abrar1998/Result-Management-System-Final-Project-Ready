...
namespace RMS.FeeReceiptRepository
{
    public class FeeReceiptService : IFeeReceiptService
    {
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                using (var document = new iTextSharp.text.Document(iTextSharp.text.PageSize.A4))
                {
                    using (var writer = PdfWriter.GetInstance(document, ms))
                    {
                        ...
                        var headerCell3 = new PdfPCell(new Phrase("Amount Paid", boldFont));
                        ...
                        var headerCell5 = new PdfPCell(new Phrase("Deposit Date", boldFont));
                        ...
                        table.AddCell(new PdfPCell(new Phrase(student.StudentName?? "N/A", boldFont)) { HorizontalAlignment = iTextSharp.text.Element.ALIGN_CENTER });
        ...
        //Download all Fee Record of Single Students
        public byte[] GenerateReceiptForStudent(int id)
        {
            // Fetch the student details from the database using the StudentId
            var student = context.Students.FirstOrDefault(s => s.StudentId == id);
            var feeDetails = context.feeDetails.Where(f => f.StudentId == id).ToList();
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                using (var document = new iTextSharp.text.Document(iTextSharp.text.PageSize.A4))
                {
                    using (var writer = PdfWriter.GetInstance(document, ms))
                    {
                        ...
                        {
                            table.AddCell(new PdfPCell(new Phrase(item.StudentId.ToString(), boldFont)) { HorizontalAlignment = iTextSharp.text.Element.ALIGN_CENTER });
                            table.AddCell(new PdfPCell(new Phrase(item.Student?.StudentName ?? "N/A", boldFont)) { HorizontalAlignment = iTextSharp.text.Element.ALIGN_CENTER });
                            table.AddCell(new PdfPCell(new Phrase(item.TotalFee, boldFont)) { HorizontalAlignment = iTextSharp.text.Element.ALIGN_CENTER });
                            table.AddCell(new PdfPCell(new Phrase(item.Description ?? "N/A", boldFont)) { HorizontalAlignment = iTextSharp.text.Element.ALIGN_CENTER });
                            table.AddCell(new PdfPCell(new Phrase(item.RegistrationDate.ToString(), boldFont)) { HorizontalAlignment = iTextSharp.text.Element.ALIGN_CENTER });
                        }
